...
| 52 | 2026-05-29 | Docs | Create per-command Prompt Commands Documentation (Markdown + PDF) | Distribution-ready, card-style log for stakeholders. | `docs/Match_Hire_Project_Prompt_Commands_Documentation.{md,pdf}` + `render-prompts-pdf.js`. `marked` added to root devDeps. | Completed |
| 53 | 2026-05-29 | Home page | Step 1 of multi-step improvements plan — personalised, role-aware homepage. Add: live hiring stats (4 metrics), trending skills, recommended companies (candidate-personalised), salary explorer preview (multi-currency), career resources, and an employer hiring snapshot block. Do NOT replace existing homepage; only add. | Business-aligned landing surface that adapts to who's looking (guest / candidate / employer). | Commit `1ce8bc6`. Extended `Backend/src/services/home.service.js` with five new aggregator helpers (`liveStats`, `trendingSkills`, `salaryExplorer`, `recommendedCompaniesFromMatches`, `employerSummary`); new `Backend/src/services/career-resources.service.js` (AI-swappable static content). `GET /api/v1/home` payload extended with `liveStats`, `trendingSkills`, `salaryExplorer`, `careerResources`, `recommendedCompanies`, `employer`. New React components in `Frontend/src/pages/Home.jsx`: LiveStatsBand, TrendingSkillsRow, RecommendedCompaniesBlock, SalaryExplorerBlock, CareerResourcesBlock, EmployerHomeSummary. CSS additions in `Frontend/src/styles.css` using existing tokens. Swagger schema updated. New tests in `qa/api/home.test.js` (4 passing). Multi-currency salary preview picks the dominant currency per label. No DB migrations. | Completed |
| 54 | 2026-05-30 | Application flow + Search UX | Step 2 of multi-step improvements plan. Applied jobs disappear from every public listing (Home, Jobs, Search, Recommended, Similar, Matching, dashboard rail). Withdrawn jobs reappear in those lists. New Withdrawn Applications sidebar tab. Withdraw flow moves from list to Job Detail. Company dashboard never shows withdrawn applications. Modern Jobs search bar (Job title / Skills / Company / Location, autocomplete via /skills?search=, recent searches in localStorage, no internal scrollbars). | Cleaner candidate experience; less noise; clearer separation between active and pulled-out pipeline; employer-side clarity (only actionable rows surface). | Commit `e075f7a`. Shared SQL fragment `notHasActiveApplicationFragment()` filters `status NOT IN ('withdrawn','rejected')` across `listPublic`, `listLocationBased`, `recommendedForUser`, `similarJobs`. `listForCandidate` accepts `statuses[]` / `exclude_statuses[]`. Validator gains `company` on both home + public job-query schemas. New `CandidateWithdrawn.jsx` page + route. `JobDetail` owns the withdraw modal + sets `is_applied` to **active-only** semantics so the Apply button returns after withdrawal. `DashboardCompany` drops the `withdrawn` status mapping + tightens terminal-status set. `Jobs.jsx` gets a modern unified search bar (4 fields, skill autocomplete, recent-searches strip, per-field clear, search icon); sidebar drops duplicate inputs + `overflow-y:auto`. **Supersedes Step 44** in-list withdraw affordance (modal moved) and the Step 44 employer "Withdrawn" pill (employers no longer see withdrawn at all). Full Playwright suite 94/94; API suite 22/22. No DB migrations. | Completed |
| 55 | 2026-05-30 | UI/UX (Job card consistency) | Standardise every job card project-wide. Restore the equal-height contract that Step 48 had auto-disabled: `.jobs-grid` back to `align-items:stretch` + `> *{height:100%}`; `.card-shell` back to `min-height:340px;height:100%`; reserved min-heights on title (48px), summary (36px), trust (22px), meta (22px), why-list (60px), tags (22px). JobCard always-renders the meta/tags/why/summary slots so the reserved heights have something to occupy. New thin wrappers `CandidateJobCard.jsx` + `CompanyJobCard.jsx` pre-bind the `viewer` prop; `DashboardCompany.jsx` migrated as first caller. QA test reverts to equal-row-heights assertion. | One identical card design everywhere so the catalogue reads as a clean SaaS grid and the Apply button lines up across every row. | Commit `a441241`. **Supersedes Step 48's auto-height contract** — the user's earlier "remove blank space" instruction is reversed in favour of "every card identical, Apply aligned"; both instructions were the user's, equal-height now wins. No changes to logic, APIs, or DB. | Completed |
| 56 | 2026-05-31 | UI/UX (Salary + Dashboard cards) | Monthly salary display everywhere ("PKR 500,000/month") via centralised `formatSalary(min, max, currency, period)` in `api/adapters.js`. Honours `salary_period`: monthly → as-is; everything else → /12. Locale-formatted with `toLocaleString`. Currency mapping: USD → `$`, others → ISO-code prefix. The retired local `formatSalary` copies in `MatchingJobsPanel.jsx` and `MatchingJobsCarousel.jsx` now delegate to the shared helper. **CandidateWithdrawn.jsx** rewritten to render through the shared `<JobCard />` inside a `.jobs-grid` (same design as the Jobs page) with Reapply CTA + Withdrawn pill + applied/withdrawn dates above each card; the custom `.withdrawn-card` row layout retired. Backend `application.repository.listForCandidate` SELECT now also returns `j.description` so the JobCard summary slot has content on Applications + Withdrawn tabs. | Pay numbers are immediately legible without mental division; dashboard cards now read identically to every other catalogue surface. | Commit `c60c417`. **Supersedes Step 33's "K" salary shorthand** (full locale formatting now) and **supersedes Step 48's custom `.withdrawn-card` design** introduced for the Withdrawn tab (replaced with the shared JobCard). No DB / API / matching changes — display-only. | Completed |
| 57 | 2026-05-31 | UI/UX + Seeded-data hygiene + Git safety | (a) Jobs salary filter switched from USD-yearly to PKR-monthly bands (`PKR 50,000 – 100,000 / month`, etc.); values sent to backend are PKR-annual equivalents. (b) `seed.bulk.js` + `seed.industries.js` now write `NULL` into `candidate_profiles.education` instead of synthesised "BS in X · Y University · YYYY"; new `Backend/src/database/scripts/clear-seeded-education.js` clears existing seeded rows (dry-run by default, `--apply` to commit; 220 rows cleared from dev DB). (c) `.why-list-empty { min-height:0; max-height:0; margin-bottom:0 }` collapses the empty 60px slot on dashboard cards (Applications / Saved / Favourites / Withdrawn — no match data → no blank band below work-mode). (d) Git identity verified: local + global remain `Azeem Akram <azeembscs86@gmail.com>`. | Filter speaks the visitor's currency; candidate profile shows ONLY what the candidate typed; no blank band on dashboard cards; contributor list stays clean. | Commit `4e6b332`. **Salary filter is still currency-blind on the backend** — PKR-monthly bands convert × 12 to match PKR-annual rows, but USD-stored jobs won't match unless the numeric bounds happen to align. Currency-aware filtering is a follow-up. No DB migrations; cleanup is a one-off UPDATE via the new script. | Completed |
| 58 | 2026-05-31 | Application workflow + Rejection feedback | Renamed "Job Applications" → "My Applications" across sidebar / dashboard overview / withdrawn empty-state. Six summary cards on the My Applications page (Total / Under Review / Shortlisted / Interview Scheduled / Accepted / Rejected) with `rollupStats` mapping the by-status counts. Sidebar gains a live Withdrawn Applications badge from `stats.applications.by_status.withdrawn`; the My-Applications badge now excludes withdrawn so sidebar + page totals agree. Interview status label standardised to "Interview Scheduled" everywhere. Rejection-reason validator now requires a canonical key (`skills_mismatch`, `insufficient_experience`, `education_mismatch`, `salary_mismatch`, `position_filled`, `location_mismatch`, `incomplete_profile`, `incomplete_application`, `poor_interview`, `other`); `custom_reason` mandatory when `other`. Controller composes the stored shape (canonical key, or `other:<text>`). Candidate list now SELECTs `rejection_reason`. New `data/rejection-reasons.js` owns key→label + improvement suggestions + a `parseRejectionReason` decoder for legacy free-text rows. New `.rejection-feedback` panel renders below the JobCard on rejected rows showing the reason + the suggestions. | Active pipeline reads at a glance; candidates get actionable feedback instead of a silent rejection; counters live-update from a single endpoint. | Commit `1aa758e`. No DB migrations — `rejection_reason` column already VARCHAR(500). Frontend + backend enum lists must stay in sync — `Frontend/src/data/rejection-reasons.js` mirrors `Backend/src/validators/employer.validator.js#REJECTION_REASON_KEYS`. | Completed |
| 59 | 2026-05-31 | Candidate Dashboard UX overhaul | (a) `.why-list-empty` 60px reservation **restored** — Favs/Saved cards match Jobs-page height exactly (supersedes Step 57's collapse). (b) `JobCard` gains `onWithdraw` + `withdrawingId` props; when supplied with `applied=true`, the Applied pill is replaced with a "Withdraw Application" outline CTA. (c) `CandidateApplications.jsx` wires onWithdraw → `.confirm-overlay` modal → optimistic remove + summary-card refetch + inline notice. (d) New `pages/CandidateRejected.jsx` + `/dashboard/candidate/rejected` route + sidebar entry with live count badge from `stats.applications.by_status.rejected`. Renders rejected rows through the shared JobCard with the `.rejection-feedback` panel underneath. (e) Sidebar gains a `.dash-side-strength` widget (profile % + progress bar + "Complete profile →" CTA when <80%) directly under the user identity — replaces the inline stat card on the overview. (f) `DashboardCandidate.jsx` overview redesigned: old `.stat-row` (4 cards) + inline "My Applications" list **removed**; new `.overview-summary` grid renders 6 `<Link>` tiles (Applications · Saved Jobs · Shortlisted · Interviews · Withdrawn · Rejected). (g) Header / mobile-drawer Favourites confirmed already removed (Step 49, commit `776e54d`). (h) `Preferences.jsx` scroll-tracker **rewritten on IntersectionObserver** (the user asked for it by name) — trigger band `rootMargin: '-15% 0px -75% 0px'`, last-intersecting-section wins, scroll-position fallback for above-first / below-last, 700ms click-lock preserved. | Cleaner overview, one summary tile per workflow surface; rejection feedback finally lives in its own surface; profile strength visible from every tab; cards visually consistent. | Commit `40964ab`. **Supersedes Step 57's why-list collapse** and **Step 50's anchor-line scroll-tracker** — both replaced by the new behaviour. Followup worth opening: score favourites + saved-jobs in the backend so the why-list populates with content instead of staying as a reserved-but-empty 60px slot. No DB / API contract changes. Full Playwright suite 94/94; preferences-scroll spec 4/4. | Completed |
| 60 | 2026-05-31 | Job-card spacing across viewer roles (final resolution) | Closes the long-running "equal heights vs no empty space" tension. (a) **CSS** — `.why-list-empty` collapses to zero (visitor + company cards never have match content by spec; the 60px reserved band was always empty for them). Within each viewer's grid, cards are uniformly collapsed so equal-height holds locally. (b) **Backend** — `candidate.service.listFavorites` + `savedJob.service.list` + `candidate.service.listApplications` now decorate rows via `jobMatchService.rankJobs(rows, candidate, { filter: false })` after the repo fetch. Live scoring against the candidate's current context populates `match_score` + `reasons` + `missing` on every candidate dashboard surface. Applications-page re-sorts by applied-at desc after decoration. (c) **Frontend** — `CandidateApplications`, `CandidateWithdrawn`, `CandidateRejected` view-models forward `match_score` / `reasons` / `missing` into `toJobCardShape`. Favourites + Saved already pass full records through. Net: visitor/company cards compact, candidate cards uniform-height across every surface with populated why-list. | One coherent card design with the right shape for each viewer role. | Commit `(this commit)`. **Resolves the iteration between Steps 48 / 55 / 57 / 59** — the equal-heights vs no-empty-band conflict is removed by ensuring source data matches at the data layer rather than fighting it in CSS. Live scoring cost: ~5–20 jobs × match algorithm per request on Favs / Saved / Applications. No DB / API contract changes. Full Playwright suite 94/94; API suite 23/23. | Completed |
